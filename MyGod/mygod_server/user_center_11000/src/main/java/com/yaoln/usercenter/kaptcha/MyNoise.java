 ...
 * 图片验证码干扰线实现类
 */
public class MyNoise extends Configurable implements NoiseProducer
{
	/**
	 * Draws a noise on the image. The noise curve depends on the factor values.
	 * MyNoise won't be visible if all factors have the value > 1.0f
	 * 
	 * @param image
			...
			float factorTwo, float factorThree, float factorFour)
	{
		...
		for (i = 0; i < 10; i++)
		{
			...
			graph.setColor(ColorUtil.getRandomColor());
